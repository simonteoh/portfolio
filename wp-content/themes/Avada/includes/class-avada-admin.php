...
class Avada_Admin {
	...
	public function admin_menu() {
		...
		if ( current_user_can( 'switch_themes' ) ) {
			...
			$dashboard         = $avada_menu_page_creation_method( 'Avada', 'Avada', 'switch_themes', 'avada', [ $this, 'dashboard_screen' ], 'none', '2.111111' );
			...
			if ( array_key_exists( 'avada', $submenu ) ) {
				foreach ( $submenu['avada'] as $key => $value ) {
					$k = array_search( 'avada-maintenance', $value );
					if ( $k ) {
	...
	public static function get_admin_screens_header( $screen = 'welcome' ) {
		...
		if ( in_array( $screen, [ 'builder-options', 'layout-sections', 'layouts', 'icons', 'library' ] ) ) {
			$screen_classes .= ' fusion-builder-wrap';

			if ( 'builder-options' === $screen ) {
				$screen_classes .= ' fusion-builder-settings';
			}
		} elseif ( in_array( $screen, [ 'sliders', 'slides', 'slide-edit' ] ) ) {
				...
				<div class="avada-db-header-main-container">
					...
					<nav class="avada-db-menu-main">
						<ul class="avada-db-menu">
							<li class="avada-db-menu-item"><a class="avada-db-menu-item-link<?php echo ( 'to' === $screen || 'builder-options' === $screen ) ? ' avada-db-active' : ''; ?>" href="<?php echo esc_url( ( 'to' === $screen ) ? '#' : admin_url( 'themes.php?page=avada_options' ) ); ?>" ><span class="avada-db-menu-item-text"><?php esc_html_e( 'Options', 'Avada' ); ?></span></a>
							...
							<li class="avada-db-menu-item"><a class="avada-db-menu-item-link<?php echo ( 'prebuilt-websites' === $screen ) ? ' avada-db-active' : ''; ?>" href="<?php echo esc_url( ( 'prebuilt-websites' === $screen ) ? '#' : admin_url( 'admin.php?page=avada-prebuilt-websites' ) ); ?>" ><span class="avada-db-menu-item-text"><?php esc_html_e( 'Websites', 'Avada' ); ?></span></a></li>
							<li class="avada-db-menu-item"><a class="avada-db-menu-item-link<?php echo ( in_array( $screen, [ 'patcher', 'plugins', 'support', 'status' ] ) ) ? ' avada-db-active' : ''; ?>" href="<?php echo esc_url( ( 'patcher' === $screen ) ? '#' : admin_url( 'admin.php?page=avada-patcher' ) ); ?>"><span class="avada-db-menu-item-text"><?php esc_html_e( 'Maintenance', 'Avada' ); ?></span><span class="avada-db-maintenance-counter"></span></a>
								<ul class="avada-db-menu-sub avada-db-menu-sub-maintenance">
									<li class="avada-db-menu-sub-item">
										<a class="avada-db-menu-sub-item-link avada-db-menu-sub-item-patcher<?php echo ( 'patcher' === $screen ) ? ' avada-db-active' : ''; ?>" href="<?php echo esc_url( ( 'patcher' === $screen ) ? '#' : admin_url( 'admin.php?page=avada-patcher' ) ); ?>">
			...
			<header class="avada-db-header-sticky">
				<div class="avada-db-menu-sticky">
					<div class="avada-db-menu-sticky-label<?php echo ( Avada()->registration->is_registered() ) ? ' completed' : ''; ?>">
						<span class="avada-db-version">
							<span><?php esc_html_e( 'v', 'Avada' ); ?><?php echo esc_html( AVADA_VERSION ); ?></span> <?php esc_html_e( '|', 'Avada' ); ?>
						</span>
	 ...
	 */
	public static function get_dashboard_screen_video_url( $is_update ) {
		...
		$transient_name = 'avada_dashboard_video_url_' . md5( $api_url );
