	...
	jQuery( document ).ready( function() {
		...
		FusionPageBuilder.LibraryView = window.wp.Backbone.View.extend( {
			...
			events: {
				...
				'click .fusion-builder-demo-header-button-load': 'loadDemoHeader',
			...
			loadDemoHeader: function( event ) {
				...
				FusionApp.confirmationPopup( {
					title: fusionBuilderText.import_demo_header,
					content: fusionBuilderText.importing_demo_header,
					actions: [
						{
							label: fusionBuilderText.cancel,
							classes: 'no',
						...
						},
						{
							...
							callback: function() {
								...
								headerName = jQuery( event.currentTarget ).data( 'header-name' );
								postId   = jQuery( event.currentTarget ).data( 'post-id' );
								...
								jQuery.ajax( {
									...
									url: fusionAppConfig.ajaxurl,
									data: {
										action: 'fusion_builder_load_demo_header',
										fusion_load_nonce: fusionAppConfig.fusion_load_nonce,
